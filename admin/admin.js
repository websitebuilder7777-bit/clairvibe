 ...
 * Dynamic portfolio category manager (Add/Delete/Edit sections),
 * visual photo replacement & deletion with side-by-side previews,
 * real-time live canvas sync, color palette & gradient customizer,
 * offline support & 1-click GitHub Pages publishing.
 */
...
(function () {
  ...
  // Default Template Data (ensures 100% offline & fallback compatibility)
  ...
  let activeTab = 'hero';
  let pendingUploads = {}; // file path -> base64 data string
  let currentReplaceCallback = null;
  let currentDeleteCallback = null;
  let tempReplaceFile = null;

  // New Category creation & deletion temporary objects
  let tempNewCatCover = null;
  let tempNewCatPhotos = [];
  ...
  async function tryFetchServerState() {
    try {
      ...
      // Offline / file:/// protocol: perfectly fine, state is pre-initialized
    }
  }
  ...
  function ensurePortfolioCategories() {
    if (!state.portfolio) state.portfolio = {};
    if (!Array.isArray(state.portfolio.categories) || state.portfolio.categories.length === 0) {
      const cats = [];
      const p = state.portfolio;
      if (p.machines) cats.push({ id: 'machines', title: p.machines.title || 'Heavy Machinery & Stage Rigs', cover: p.machines.cover || 'machines/M.png', images: p.machines.images || [] });
      if (p.food) cats.push({ id: 'food', title: p.food.title || 'Food & FMCG', cover: p.food.cover || 'food and fmcg/F.png', images: p.food.images || [] });
      if (p.corporate) cats.push({ id: 'corporate', title: p.corporate.title || 'Corporate & Brand Summits', cover: p.corporate.cover || 'coorporate/C.png', images: p.corporate.images || [] });
      
      if (cats.length === 0) {
        state.portfolio.categories = JSON.parse(JSON.stringify(DEFAULT_CONTENT.portfolio.categories));
      } else {
        state.portfolio.categories = cats;
      }
    }
  }
  ...
  function updateBadges() {
    ensurePortfolioCategories();
    (state.portfolio.categories || []).forEach(cat => {
      const b = document.getElementById(`badge_${cat.id}`);
      if (b) b.textContent = (cat.images || []).length;
    });
  }
  ...
  /* ── 2. Dynamic Portfolio Categories Panel (Add/Edit/Delete Categories) ── */
  function renderPortfolioCategories() {
    ensurePortfolioCategories();
    const categories = state.portfolio.categories || [];
    const sidebarContainer = document.getElementById('sidebarGalleryTabs');
    const panelsContainer = document.getElementById('dynamicCategoryPanels');

    if (!sidebarContainer || !panelsContainer) return;

    // A. Render Sidebar Gallery Tabs
    sidebarContainer.innerHTML = '';
    categories.forEach(cat => {
      const tabEl = document.createElement('div');
      const isTabActive = activeTab === ('gallery_' + cat.id);
      tabEl.className = `sidebar-tab ${isTabActive ? 'active' : ''}`;
      tabEl.dataset.tab = 'gallery_' + cat.id;
      tabEl.innerHTML = `
        ...
        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${cat.title || 'Untitled Section'}</span>
        <span class="badge" id="badge_${cat.id}">${(cat.images || []).length}</span>
      `;
      sidebarContainer.appendChild(tabEl);
    });

    // B. Render Dynamic Editor Panels
    panelsContainer.innerHTML = '';
    categories.forEach(cat => {
      const isPanelActive = activeTab === ('gallery_' + cat.id);
      const panel = document.createElement('section');
      panel.className = `tab-content ${isPanelActive ? 'active' : ''}`;
      panel.id = `tab_gallery_${cat.id}`;

      const coverSrc = resolvePath(cat.cover);

      panel.innerHTML = `
        <div class="content-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.75rem">
          <div>
            <h2 class="content-title" id="heading_title_${cat.id}">${cat.title || 'Untitled Section'}</h2>
            <p class="content-subtitle">Manage card cover, gallery photos, and section title.</p>
          </div>
          <button class="btn btn-ghost btn-sm" id="btnDeleteCat_${cat.id}" style="color:var(--red);border-color:rgba(239,68,68,0.3);cursor:pointer;display:flex;align-items:center;gap:0.35rem" title="Delete entire section">
        ...
        <div class="editor-card">
          ...
          <div class="form-group" style="margin:0">
            <label class="form-label">Category Name (Shown on portfolio cards and lightbox titles)</label>
            <input type="text" class="form-input" id="input_title_${cat.id}" value="${cat.title || ''}" placeholder="e.g. Luxury Weddings & Galas">
          </div>
        </div>

        <!-- Featured Card Cover Slot -->
        <div class="editor-card">
          <div class="card-heading">
            <span>Featured Card Cover Photo</span>
          </div>
          <div id="coverCard_${cat.id}">
            <div class="visual-image-card">
              <div class="image-preview-box">
                ...
                <div class="image-hover-actions">
                  <button class="action-btn-pill pill-replace" id="btnReplaceCover_${cat.id}">
        ...
        <div class="editor-card">
          <div class="card-heading">
            <span>Popup Gallery Photos</span>
            <span class="card-counter" id="counter_${cat.id}">${(cat.images || []).length} Photos</span>
          </div>
          <div class="visual-gallery-grid" id="grid_${cat.id}"></div>
        </div>
      `;
      ...
      // Title input binding
      const titleInput = panel.querySelector(`#input_title_${cat.id}`);
      if (titleInput) {
        titleInput.oninput = (e) => {
          cat.title = e.target.value;
          const heading = document.getElementById(`heading_title_${cat.id}`);
          if (heading) heading.textContent = cat.title || 'Untitled Section';
          const tabSpan = sidebarContainer.querySelector(`[data-tab="gallery_${cat.id}"] span`);
          if (tabSpan) tabSpan.textContent = cat.title || 'Untitled Section';
          onStateChanged();
        };
      }

      // Cover Replace binding
      const btnRepCover = panel.querySelector(`#btnReplaceCover_${cat.id}`);
      if (btnRepCover) {
        btnRepCover.onclick = () => {
          openReplaceModal({
            title: `Replace Card Cover — ${cat.title || 'Section'}`,
      ...
      // Delete Section button binding
      const btnDelCat = panel.querySelector(`#btnDeleteCat_${cat.id}`);
      if (btnDelCat) {
        btnDelCat.onclick = () => {
      ...
      // Render gallery photos for this category
      renderCategoryPhotosGrid(cat);
    });

    setupSidebarTabs();
  }

  /* Render Gallery Photos inside a specific category */
  function renderCategoryPhotosGrid(cat) {
    const gridContainer = document.getElementById(`grid_${cat.id}`);
    ...
    addBox.innerHTML = `
      ...
      <input type="file" multiple accept="image/*" style="display:none" id="multiFileInput_${cat.id}">
    `;

    const multiFileInput = addBox.querySelector(`#multiFileInput_${cat.id}`);
    ...
    addBox.ondrop = async (e) => {
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        await handleAddMultipleFilesToCategory(cat, Array.from(e.dataTransfer.files));
      }
    };

    multiFileInput.onchange = async (e) => {
      if (e.target.files && e.target.files.length > 0) {
        await handleAddMultipleFilesToCategory(cat, Array.from(e.target.files));
      }
    };
    ...
    // Update counter
    const counter = document.getElementById(`counter_${cat.id}`);
    if (counter) counter.textContent = `${(cat.images || []).length} Photos`;
  }

  async function handleAddMultipleFilesToCategory(cat, files) {
    if (!cat) return;
    if (!Array.isArray(cat.images)) cat.images = [];

    for (const file of files) {
      const base64 = await fileToBase64(file);
      const cleanPath = `uploads/${cat.id}_${Date.now()}_${sanitizeFilename(file.name)}`;
  ...
  /* ─────────────────────────────────────────────────────────────
     CATEGORY MODALS (CREATE & DELETE WHOLE SECTIONS)
     ───────────────────────────────────────────────────────────── */
  function setupCategoryModals() {
    // 1. Add Category Modal
    const btnOpenAdd = document.getElementById('btnOpenAddCategoryModal');
    const modalAdd = document.getElementById('createCategoryModal');
    const closeAdd = document.getElementById('createCategoryModalClose');
    const cancelAdd = document.getElementById('btnCancelCreateCat');
    const confirmAdd = document.getElementById('btnConfirmCreateCat');
    const inputTitle = document.getElementById('newCatTitleInput');
    const inputCover = document.getElementById('newCatCoverInput');
    const inputPhotos = document.getElementById('newCatPhotosInput');
    const coverText = document.getElementById('newCatCoverText');
    const photosText = document.getElementById('newCatPhotosText');
    const coverPreviewBox = document.getElementById('newCatCoverPreviewBox');
    const coverPreviewImg = document.getElementById('newCatCoverPreviewImg');

    const openAddModal = () => {
      tempNewCatCover = null;
      tempNewCatPhotos = [];
      if (inputTitle) inputTitle.value = '';
      if (inputCover) inputCover.value = '';
      if (inputPhotos) inputPhotos.value = '';
      if (coverText) coverText.textContent = 'Click or Drop Card Cover Photo';
      if (photosText) photosText.textContent = 'Select or Drop Initial Photos';
    ...
    if (closeAdd) closeAdd.onclick = closeAddModal;
    if (cancelAdd) cancelAdd.onclick = closeAddModal;

    if (inputCover) {
      inputCover.onchange = async (e) => {
        const file = e.target.files[0];
        if (file) {
          const base64 = await fileToBase64(file);
          const cleanPath = `uploads/cover_${Date.now()}_${sanitizeFilename(file.name)}`;
          tempNewCatCover = { path: cleanPath, base64: base64 };
          if (coverPreviewImg) coverPreviewImg.src = base64;
          if (coverPreviewBox) coverPreviewBox.style.display = 'block';
          if (coverText) coverText.textContent = `Selected: ${file.name}`;
        }
      };
    }

    if (inputPhotos) {
      inputPhotos.onchange = async (e) => {
        if (e.target.files && e.target.files.length > 0) {
          tempNewCatPhotos = [];
          for (const file of e.target.files) {
            const base64 = await fileToBase64(file);
            const cleanPath = `uploads/photo_${Date.now()}_${sanitizeFilename(file.name)}`;
            tempNewCatPhotos.push({ path: cleanPath, base64: base64 });
          }
          if (photosText) photosText.textContent = `${e.target.files.length} photo(s) selected`;
        }
      };
    }
    ...
    if (confirmAdd) {
      confirmAdd.onclick = () => {
        ...
        let coverPath = 'https://placehold.co/800x600/15154A/F5A623?text=' + encodeURIComponent(title);
        const imagesList = [];

        if (tempNewCatCover) {
          coverPath = tempNewCatCover.path;
          pendingUploads[tempNewCatCover.path] = tempNewCatCover.base64;
          imagesList.push(tempNewCatCover.path);
        }

        if (tempNewCatPhotos.length > 0) {
          tempNewCatPhotos.forEach(p => {
            pendingUploads[p.path] = p.base64;
            if (!imagesList.includes(p.path)) {
              imagesList.push(p.path);
            }
          });
        }

        if (imagesList.length === 0 && coverPath) {
          imagesList.push(coverPath);
        }

        ensurePortfolioCategories();
        state.portfolio.categories.push({
          id: newId,
          title: title,
          cover: coverPath,
          images: imagesList
        ...
        activeTab = 'gallery_' + newId;
        closeAddModal();
        renderPortfolioCategories();
        onStateChanged();
        showToast(`Created new section "${title}"!`, 'success');
      };
    }

    // 2. Delete Category Modal
    const modalDel = document.getElementById('deleteCategoryModal');
    const closeDel = document.getElementById('deleteCategoryModalClose');
    const cancelDel = document.getElementById('btnCancelDeleteCat');
    const confirmDel = document.getElementById('btnConfirmDeleteCat');

    const closeDelModal = () => {
      if (modalDel) modalDel.classList.remove('open');
    ...
    if (confirmDel) {
      confirmDel.onclick = () => {
        ...
        const catTitle = categoryToDelete.title;

        state.portfolio.categories = state.portfolio.categories.filter(c => c.id !== catId);

        if (activeTab === 'gallery_' + catId) {
          if (state.portfolio.categories.length > 0) {
            activeTab = 'gallery_' + state.portfolio.categories[0].id;
          } else {
            activeTab = 'hero';
          }
        }
        ...
        renderPortfolioCategories();
        onStateChanged();
        showToast(`Deleted section "${catTitle}"`, 'success');
      };
    }
  }
  ...
  function openDeleteCategoryModal(cat) {
    ...
    const modalDel = document.getElementById('deleteCategoryModal');
    const delThumb = document.getElementById('delCatThumb');
    const delTitle = document.getElementById('delCatTitle');
    const delCount = document.getElementById('delCatCount');

    if (delThumb) delThumb.src = resolvePath(cat.cover);
    if (delTitle) delTitle.textContent = cat.title || 'Untitled Section';
    if (delCount) delCount.textContent = `${(cat.images || []).length} photos in this section`;

    if (modalDel) modalDel.classList.add('open');
  }

  /* ─────────────────────────────────────────────────────────────
     MEDIA REPLACEMENT & DELETION MODALS (SIDE-BY-SIDE PREVIEWS)
     ───────────────────────────────────────────────────────────── */
  ...
  /* ─────────────────────────────────────────────────────────────
     VIEW MODES & SIDEBAR TABS SWITCHER
     ───────────────────────────────────────────────────────────── */
  function setupViewModeSwitchers() {
    ...
    // Device toggles for canvas preview
  ...
  function setupSidebarTabs() {
    ...
    // Ensure active tab class is synchronized
    tabs.forEach(tab => {
  ...
  function setupGlobalButtons() {
    ...
    // 2. Publish Button (1-Click GitHub Repository Save)
    ...
    if (btnPublish) {
      btnPublish.onclick = async () => {
        ...
        try {
          // Upload any pending media base64 files
